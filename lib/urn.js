  ...
  check: function(urns) {
    ...
    for (var index in urns) {
      ...
      if (!urn.match(allowedProtocolsRegExp) || !urn.match(/^([^:]+:)+[^:]+$/)) {
        return 'No matched protocols: Incorrect protocol provided';
      }
        ...
        case 'pbk:ec:secp256r1':
          ...
          if (charArr[0] === '02' || charArr[0] === '03') {
            ...
            if (charArr.length !== 65) {
              return 'Incorrect uncompressed public key byte length: Uncompressed keys should be 1 byte + 32 bytes for 'x' coordinate + 32 bytes for 'y' coordinate';
            }
          }
