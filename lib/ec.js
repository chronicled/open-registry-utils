...
module.exports = {
  ...
  verify: function(identityURN, hashHex, sigHex, _curve) {
    var curve = _curve || 'secp256r1';
    var publicKeyHex = this.getPlainPubKey(urn.split(identityURN).id);
    var ec = new jsrsasign.ECDSA({'curve': curve});
    return ec.verifyHex(hashHex, sigHex, publicKeyHex);
  },

  isCompressed: function(publicKey) {
    var firstByte = publicKey.slice(0, 2);
    if (firstByte == '04') {
      return false;
    } else if ({'02': 1, '03': 1}[firstByte]) {
      return true;
    } else {
      throw new Error('Incorrect format of the public key');
    }
  },

  getPlainPubKey: function(publicKey) {
    return this.isCompressed(publicKey) ? this.decompress(publicKey) : publicKey;
  },
